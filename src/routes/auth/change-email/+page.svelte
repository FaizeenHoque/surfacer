<script lang="ts">
  ...
  onMount(async () => {
    const store = await authStore.checkUser();
    if (store) {
...
<div class="grain min-h-screen flex items-center justify-center p-4" style="background:#09090d">
  <div class="w-full max-w-md">
    ...
    <!-- Heading -->
    <h1 class="text-2xl font-semibold text-white text-center mb-2">Change email</h1>
    <p class="text-sm text-center mb-8" style="color:#4a4a5e">Current email: {currentEmail}</p>

    <!-- Form -->
    <div class="space-y-4 rounded-2xl p-6" style="background:#111116; border:1px solid #ffffff0d">
      <!-- New Email -->
      <div>
        <label for="change_email" class="text-xs font-mono uppercase" style="color:#4a4a5e">New Email</label>
        <input
          id="change_email"
          type="email"
          bind:value={newEmail}
      ...
      <div class="relative">
        ...
        <div class="relative flex justify-center text-xs">
          <span class="px-2" style="background:#111116; color:#4a4a5e">Or</span>
        </div>
      </div>
      ...
      <button
        ...
        style="border:1px solid #ff6b6b4d; color:#ff8787"
        onmouseenter={e => (e.currentTarget as HTMLElement).style.backgroundColor = '#ff6b6b14'}
