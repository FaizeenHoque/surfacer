...
<div class="grain app-shell min-h-dvh h-dvh flex overflow-hidden font-sans text-white" style="background:#09090d">
  ...
  >
    ...
    <div class="p-4 flex flex-col gap-3" style="border-top:1px solid #ffffff0d">
      <!-- Plan Name (if subscription active) -->
      {#if activeSubscription}
        <div class="px-3 py-2 rounded-lg" style="background:#00e5a008">
          <p class="text-[10px] font-mono" style="color:#7a7f9f">Current plan</p>
          <p class="text-xs font-semibold text-white capitalize mt-0.5">{activeSubscription.productName || activeSubscription.status}</p>
        </div>
      {/if}
      ...
      {#if currentUser}
        ...
        >
          ...
          <svg class="w-3.5 h-3.5 shrink-0" style="color:#7a7f9f" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6h12.75A2.25 2.25 0 0121 8.25v10.5A2.25 2.25 0 0118.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6m0 0A2.25 2.25 0 015.25 3h13.5A2.25 2.25 0 0121 5.25m-21 0v.006L3 6"/>
          </svg>
        </button>

        <!-- Settings Modal Centered -->
        {#if isSettingsMenuOpen}
          <div class="fixed inset-0 z-40 animate-fade-in bg-black/60 backdrop-blur-sm" role="button" tabindex="0" onclick={() => { isSettingsMenuOpen = false; }} onkeydown={(e) => { if (e.key === 'Escape') isSettingsMenuOpen = false; }}></div>
          
          <div class="fixed inset-0 z-50 flex items-center justify-center p-4 animation-scale-up">
            <div class="w-full max-w-4xl max-h-[85vh] rounded-2xl border overflow-hidden flex flex-col" style="background:#09090d; border-color:#ffffff14">
              
              <!-- Modal Header -->
              <div class="shrink-0 px-6 py-5 flex items-center justify-between border-b" style="border-color:#ffffff08">
                <div>
                  <h2 class="text-lg font-semibold text-white">Settings</h2>
                  <p class="text-sm font-mono mt-0.5" style="color:#7a7f9f">{currentUser.email || ''}</p>
                </div>
                <button
                ...
                >
                  <svg class="w-5 h-5" style="color:#7a7f9f" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              ...
              <div class="flex-1 overflow-hidden flex min-h-0">
                ...
                <div class="w-48 shrink-0 border-r px-3 py-4 overflow-y-auto" style="border-color:#ffffff08">
                  {#each [
                    { id: 'general' as const, label: 'General', icon: 'M10.5 6h12.75' }
                  ] as navItem (navItem.id)}
                    <button
                      onclick={() => { settingsMenuTab = navItem.id; }}
                      class="w-full text-left px-4 py-3 rounded-lg transition-all flex items-center gap-3 text-sm font-medium hover:bg-white/20 active:bg-white/15"
                      style="background:{settingsMenuTab === navItem.id ? '#ffffff10' : '#ffffff08'}; color:{settingsMenuTab === navItem.id ? '#ffffff' : '#8b90a5'}"
                    >
                  ...
                  <div class="border-t mt-6 pt-4" style="border-color:#ffffff08">
                    {#each [
                      { id: 'privacy' as const, label: 'History', icon: 'M12 8v4l3 2m6-11a9 9 0 11-18 0 9 9 0 0118 0z' }
                    ] as navItem (navItem.id)}
                      <button
                        onclick={() => { settingsMenuTab = navItem.id; }}
                        class="w-full text-left px-4 py-3 rounded-lg transition-all flex items-center gap-3 text-sm font-medium hover:bg-white/20 active:bg-white/15"
                      ...
                      >
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                          <path stroke-linecap="round" stroke-linejoin="round" d={navItem.icon}/>
                        </svg>
                        {navItem.label}
                      </button>
                    {/each}
                  ...
                  <div class="mt-6 pt-0" style="border-color:#ffffff08">
                    <button
                      onclick={handleLogout}
                      class="w-full text-left px-4 py-3 rounded-lg transition-all flex items-center gap-3 text-sm font-semibold hover:bg-red-500/30 active:bg-red-500/40"
                      style="background:#ff574220; color:#ff8787; border:1px solid #ff57421a"
                    >
                      <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="flex-1 overflow-y-auto px-8 py-6">
                  {#if settingsMenuTab === 'general'}
                    ...
                    <div class="space-y-8">
                      <div>
                        <h3 class="text-sm font-semibold text-white mb-4">Preferences</h3>
                        <div class="space-y-6">
                          <!-- Model Selection -->
                          <div>
                          ...
                          <div class="flex items-center justify-between">
                            ...
                            >
                              {reasoningVisibility === 'show' ? 'On' : 'Off'}
                            </button>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <h3 class="text-sm font-semibold text-white mb-4">Billing & Subscription</h3>
                        <div class="space-y-4">
                          {#if activeSubscription}
                            <!-- Subscription Info -->
                            <div class="p-4 rounded-lg" style="background:#ffffff08; border:1px solid #ffffff0d">
                              <div class="space-y-3">
                                <div>
                                  <p class="text-xs font-mono" style="color:#7a7f9f">Plan</p>
                                  <p class="text-sm font-semibold text-white capitalize mt-1">{activeSubscription.productName || activeSubscription.status}</p>
                                </div>
                                {#if activeSubscription.nextBillingDate}
                                  <div>
                                    <p class="text-xs font-mono" style="color:#7a7f9f">Next Billing Date</p>
                                    <p class="text-sm font-semibold text-white mt-1">{new Date(activeSubscription.nextBillingDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                                  </div>
                                {/if}
                                {#if activeSubscription.cancelAtNextBillingDate}
                                  <div>
                                    <p class="text-xs font-semibold" style="color:#ff8787">📄 Cancellation scheduled at next billing date</p>
                                  </div>
                                {/if}
                              </div>
                            </div>
                            ...
                            <button
                              ...
                              class="w-full px-4 py-2.5 rounded-lg text-sm font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                              style="background:#3a1114; border:1px solid #7f1d1d; color:#fecaca"
                            >
                              {#if activeSubscription?.cancelAtNextBillingDate}
                                ✓ Cancellation scheduled
                              {:else}
                                {isCancellingSubscription ? 'Cancelling...' : '🗑️ Cancel subscription'}
                              {/if}
                            </button>
                          {:else}
                            <div class="text-center py-6" style="color:#7a7f9f">
                              <p class="text-sm">No active subscription</p>
                              <p class="text-xs mt-2">Purchase a plan to get started</p>
                            </div>
                          {/if}
                        </div>
                      </div>
                    </div>
                  {:else if settingsMenuTab === 'privacy'}
                    <!-- History -->
                    <div class="space-y-6">
                      <div>
                        <h3 class="text-sm font-semibold text-white mb-4">Extraction History</h3>
                        {#if isLoadingExtractions}
                          <div class="text-center py-8" style="color:#7a7f9f">Loading...</div>
                        {:else if extractionHistory.length === 0}
                          <div class="text-center py-12" style="color:#7a7f9f">No extraction history yet</div>
                        {:else}
                          <div class="space-y-3 max-h-96 overflow-y-auto">
                            {#each extractionHistory as extraction (extraction.id)}
                              <div class="p-4 rounded-lg border transition-all hover:border-opacity-100" style="background:#ffffff07; border-color:#ffffff0d">
                                <div class="flex items-start justify-between gap-4">
                                  <div class="flex-1 min-w-0">
                                    <p class="text-sm font-semibold text-white truncate">{extraction.file_name}</p>
                                    <p class="text-xs font-mono truncate mt-1" style="color:#7a7f9f">Q: {extraction.prompt.slice(0, 60)}...</p>
                                  </div>
  ...
  {#if isBillingModalOpen}
    ...
    <div class="fixed inset-0 z-50 grid place-items-center px-4" aria-label="Choose monthly credit plan" role="dialog" aria-modal="true">
      <div class="w-full max-w-3xl rounded-2xl p-5" style="background:#101218; border:1px solid #ffffff1a">
        ...
        <div class="mt-5">
          {#if activeSubscription}
            <div class="text-xs font-mono" style="color:#8b90a5">
              <p class="font-semibold" style="color:#00e5a0">✓ Subscription Active</p>
              <p class="mt-1">Plan: <span style="color:#c9c9d9">{activeSubscription.productName || activeSubscription.status}</span></p>
              {#if activeSubscription.cancelAtNextBillingDate}
                <p class="mt-1 font-semibold" style="color:#ff6b6b">Subscription scheduled for cancellation</p>
              {/if}
              {#if activeSubscription.nextBillingDate}
                <p>Next billing: <span style="color:#c9c9d9">{new Date(activeSubscription.nextBillingDate).toLocaleDateString()}</span></p>
              {/if}
            </div>
          {:else if selectedPack()}
            <p class="text-xs font-mono" style="color:#8b90a5">Selected: {selectedPack()?.credits} credits {selectedPack()?.interval === 'month' ? 'per month' : selectedPack()?.interval === 'year' ? 'per year' : 'one time'}</p>
          {:else}
            <p class="text-xs font-mono" style="color:#8b90a5">Select a plan to continue</p>
          {/if}
        </div>
